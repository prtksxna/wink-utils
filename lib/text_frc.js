...
//
var prepare = require( 'wink-utils/lib/prepare_text' );
var similarity = require( 'wink-utils/lib/compute_similarity' );
var ah = require( 'wink-utils/lib/array_helpers' );
var oh = require( 'wink-utils/lib/object_helpers' );
...
// Returns an array `[ function, name, optional arguments ]`.
var validateTask = function ( task, repo ) {
  // Task must be an array.
  if ( !ah.isArray( task ) ) {
    throw Error( 'textFRC: Task should be an array, instead found: ' + JSON.stringify( task ) );
  }
  // The first element should be of type string i.e. function name.
  if ( typeof task[ 0 ] !== 'string' ) {
    throw Error( 'textFRC: Function name should be a string, instead found: ' + JSON.stringify( task[ 0 ] ) );
  }
  ...
  for ( var i = 0; i < fargs.length; i += 1 ) {
    if ( ( typeof fargs[ i ] !== 'number' ) &&
         ...
         throw Error( 'textFRC: Argument should be an number/boolean, instead found: ' + JSON.stringify( fargs[ i ] ) );
    }
  }
  ...
  if ( !f ) throw Error( 'textFRC: Invalid function found: ' + JSON.stringify( name ) );
  // `tokens.removeWords` arguments are not required as `execPrepTasks` is dropping
  // the job.
  if ( ( name === 'tokens.removeWords' ) && ( fargs.length ) ) {
    throw Error( 'textFRC: tokens.removeWords should not have any arguments; but found: ' + JSON.stringify( fargs.join( ', ' ) ) );
  }
...
// appropriate error if any validation fails.
var validateTasks = function ( tasks, repo, stopWords ) {
  var f;
  var i, imax;
  var functions;
  if ( !tasks ) return [];
  if ( !ah.isArray( tasks ) ) {
    throw Error( 'textFRC: Prep Tasks should be an array, instead found: ' + JSON.stringify( tasks ) );
  }
  imax = tasks.length;
  functions = new Array( imax );
  for ( i = 0; i < imax; i += 1 ) {
    // Tasks are always checked in `prepare` repo.
    f = validateTask( tasks[ i ], repo );
    if ( !f ) return false;
    // Can not call a helper function!
    if ( ( imax ) && ( tasks[ 0 ][ 0 ][ 0 ] === 'h' ) ) {
      throw Error( 'textFRC: Invalid function usage in Prep Tasks: ' + JSON.stringify( tasks ) );
    }
    // Quick & dirty way to check that `tokens` name space follows `string`
    // name space.
    if ( ( i < ( imax - 1 ) ) && ( tasks[ i ][ 0 ][ 0 ] > tasks[ i + 1 ][ 0 ][ 0 ] ) ) {
      throw Error( 'textFRC: string function can not be used after a token function has been used: ' +
        JSON.stringify( tasks[ i + 1 ][ 0 ] ) );
    }
...
var textFRC = function ( rules ) {
  ...
  var stopWordsPrepTasks = validateTasks( rules.stopWordsPrepTasks, prepare );
  ...
  var indexPrepTasks = validateTasks( rules.indexPrepTasks, prepare, stopWords );
  var similarityPrepTasks = validateTasks( rules.similarityPrepTasks, prepare, stopWords );
  var indexUsing = ( rules.indexUsing ) ? validateTask( rules.indexUsing, prepare ) : false;
  var sdb = buildSynonymsDB( rules.synonyms, similarityPrepTasks, indexUsing, indexPrepTasks );
  var similarityMeasure = validateTask( rules.similarityMeasure, similarity );
