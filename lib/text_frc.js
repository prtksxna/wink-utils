...
// ### Text Fuzzy Rule-based Classifier
...
var textFRC = function ( rules ) {
  ...
  var predict = function ( input ) {
    ...
    // The `sdb` index.
    var index = sdb.index;
    // Current and previous similarities and the match.
    var smlrty = 0,
        prevSmlrty = 0,
        match;
    // Transformed input.
    ...
    if ( indexUsing ) {
      ...
      for ( indexKey in indexKeys ) { // eslint-disable-line guard-for-in
        ikeys = index[ indexKey ];
        if ( ikeys ) {
          // Search if `ikeys` is defined.
