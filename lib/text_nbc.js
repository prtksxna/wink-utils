...
// It is a Naive Bayes Classifier for text classification. It exposes following
// methods:
// 1. `setPreprocessors` allows to define a pipeline of functions that will be
// used to pre-process each input during *learning*, *prediction*, and *evaluation*.
...
// 9. `metrices` in terms of primarily macro-averages of *precison*, *recall*,
...
var textNBC = function () {
  ...
  var voc = new Set();
  // Preparation tasks that are executed on the `learn` & `predict` input.
  var pTasks = [];
  // And its count.
  var pTaskCount;
  ...
  // Pre-processes the `input` by building a pipeline of tasks defined variable
  // `setPreprocessors()`
  var preprocessInput = function ( input ) {
  ...
  // Set the `tasks` that need to be performed on `learn()` and `predict()`
  // inputs prior to further processing. The `tasks` should be an array of
  // functions; using these function a simple pipeline is built to serially
  // transform the input to the output. It validates the `tasks` before updating
  // the `pTasks`. If validation fails it returns `false`; otherwise it sets the
  // `pTasks` and returns `true`.
  var setPreprocessors = function ( tasks ) {
  ...
  var learn = function ( input, label, considerOnlyPresence ) {
    ...
    var tkns = preprocessInput( input );
  ...
  var predict = function ( input ) {
    ...
    for ( var i = 0; i < labelCount; i += 1 ) {
      label = labels[ i ];
      allOdds.push( [ label, odds( preprocessInput( input ), label ) ] );
    }
  ...
  methods.stats = stats;
  methods.setPreprocessors = setPreprocessors;
