...
// it defaults to **60**.
//
// It returns an object containing (a) **minimum** number of `edits` required to
// transform `s1` to `s2`, and (b) `similarity` - a number between 0 and 1.
//
...
similarity.string.createDLFunction = function ( maxLen ) {
  ...
  return ( function ( s1, s2 ) {
    ...
    if ( s2Len === 0 ) return { edits: s1Len, similarity: 0 };
    if ( s1Len === 0 ) return { edits: s2Len, similarity: 0 };
