...
// Computes the probaibility along with **range** for the CI i.e. `zscore` from the
...
// If `zscore` is not given, it defaults to value corresponding to 90% CI.
//
...
stats.probability.range4CI = function ( successCount, totalCount, zscore ) {
...
stats.histogram.hybrid = function ( sortedData, dataPrecision, rsWithMAD, accessor ) {
  ...
  if ( binWidth > 0 ) {
    ...
    if ( rs.histogram.freq.filter( function ( e ) { return ( e === 0 ); } ).length > rs.histogram.freq.length * 0.20 ) { // eslint-disable-line
