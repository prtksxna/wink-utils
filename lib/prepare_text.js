...
// each of the sentence. It uses `~` as the `splChar` for splitting and therefore
...
prepare.string.sentences = function ( s, splChar ) {
  ...
  return s.replace( '...', '…' ).replace( rgx.eosPunctuations, substitute ).split( splCh ).map( prepare.string.trim );
}; // sentences()
