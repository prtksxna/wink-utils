...
// Creates a Set of characters from the input string `s`. This is useful in even
// more aggressive string matching using Jaccard or Tversky Indexes compared to
// `marker()`.
prepare.string.set = function ( s ) {
  return ( new Set( s ) );
}; // set()
...
// The default value of `size` is 2. The `size` 0 is forced to 2.
prepare.string.bong = function ( s, size ) {
  var ng = ( size || 2 ),
...
// Defines the `feature` for which bag of ngrams of `size` along with index will
// be generated. The `feature` should be string and is typically the name of the
// column for which this function is being used. It returns 2 functions
// viz. (a) `bong()` and (b) `index()`.
prepare.string.bongWithIndex = function ( feature, size ) {
  // Returned from this function.
  var methods = Object.create( null );
  // Index builds up in `ngIndex`.
  var ngIndex = Object.create( null );
  // Initialize ngram size.
  var ng = ( size || 2 );

  // Returns the bag of ngrams of predefined `size` from sting `s`. The argument
  // `k` is the index (i.e. row number) of `feature` `s`.
  methods.bong = function ( s, k ) {
  ...
  // Returns the index and *must be called in the end* only i.e. after calls to
  // `bong()` are over.
  methods.index = function () {
    return (
      {
        // This ensures that the function that uses this index knows (a) what
        // feature was indexed, (b) which operation was performed using (c)
        // parameters. Notice the frozen index!
        feature: feature,
        index: Object.freeze( ngIndex ),
        operation: prepare.string.bong,
        params: size
      }
    );
  }; // index()
